...
class TestVMAnalyzersReporter(unittest.TestCase):
    ...
    `VMAnalyzersReporter` class.

    It inherits from `unittest.TestCase` and verifies whether the
    `start_report` method of the `VMAnalyzersReporter` class
    can correctly call related functions by mocking virtual
    machine-related objects.
    """
    def setUp(self):
        self.test_vm = {
    ...
    def test_start_report(self):
        ...
        mock_stat = vm_storage.get_stats_info
        mock_analyzer = vm_analyzer.analyze_stats
