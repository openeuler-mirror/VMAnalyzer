...
class VMCollector:
    def __init__(self, output_dir: str = "./get_vm_time_data"):
    ...
    def collect_single_vm_data(self, vm_name: str) -> Dict:
        ...
        get_time = self.call_qga_interface(vm_name, "guest-get-time")
        vm_data = {
            "name": vm_name,
            "get_vm_time": {
                "interface": "guest-get-time",
    ...
    def collect_all_vms(self):
        ...
        try:
            ...
            LOG_ERROR(f"Exception while collecting data for all VMs：{str(e)}")

    def save_data(self):
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        file_name = f"get_time_{timestamp}.json"
