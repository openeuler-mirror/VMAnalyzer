...
for vm in (SP("virsh list --name|grep -v ^$|grep -v ^-$").split()):
    devs={}
    for ln in SP(f"virsh domblklist {shlex.quote(str(vm))} --details").split("\\n")[2:]:
        p=ln.split()
        if len(p)>=3:
            s={}
            for l in SP(f"virsh domblkinfo {shlex.quote(str(vm))} {p[1]}").split("\\n"):
                if ":" in l:
                    k,v=l.split(":",1);s[k.strip()]=v.strip()
            for l in SP(f"virsh domblkstat {shlex.quote(str(vm))} {p[1]}").split("\\n"):
                kv=l.split()
